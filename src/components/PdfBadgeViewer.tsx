...
function computeMatchRect(
  ...
  const boxes = selectedItems.map((item) => {
    const startPoint = pdfjsLib.Util.transform(viewport.transform, [item.transform[4], item.transform[5]]);
    const endPoint = pdfjsLib.Util.transform(viewport.transform, [item.transform[4] + item.width, item.transform[5] + item.height]);
    return {
      left: Math.min(startPoint[0], endPoint[0]),
      top: Math.min(startPoint[1], endPoint[1]),
      right: Math.max(startPoint[0], endPoint[0]),
      bottom: Math.max(startPoint[1], endPoint[1]),
    };
  });
